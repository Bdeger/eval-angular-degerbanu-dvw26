...
const jwtInterceptor = require("./jwt-middleware");
...
app.post("/login", (req, res) => {
  ...
  connection.query("SELECT * FROM user WHERE email= ? AND password=?",
    ...
    (erreur, resultat) => {
      ...
      // user s'est trompé de login /mdp
      if (resultat.length === 0) {
        return res.status(401).send();
      }
      const user = resultat[0];

      const token = jwt.sign({ sub: req.body.email }, "mon-super-secret");
      res.send(token);
    }
  );
});
...
app.post("/supprimer-image", jwtInterceptor, (req, res) => {
  categories[req.body.indexCategorie].images.splice(req.body.indexImage, 1);

  res.status(200).send();
});

app.patch("/deplacement-image", jwtInterceptor, (req, res) => {
